  ...
  <script>
    function BeforePage(pageNow,user_id){
      if(pageNow == 1){
        ...
        const desturl = "ShowAllApprove.jsp?" + "teacher_id=" + user_id + "&" + "page=" + page
...
<table>
  ...
  <%
    var id = Integer.parseInt(request.getParameter("teacher_id"));
    int pageCount = 3;
    int count = ApplyService.getApplyService().getCountSelectedApprove(id)/pageCount + 1;
    var pageNow = Integer.parseInt(request.getParameter("page"));
    int pageNum = pageCount * (pageNow-1);
    List<Apply> applies = ApplyService.getApplyService().getAllApprove(id,pageNum,pageCount);
    if(applies != null){
